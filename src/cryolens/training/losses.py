...
class AdaptiveContrastiveAffinityLoss(nn.Module):
    """
    Adaptive contrastive affinity loss with similarity-proportional margins.
    
    This loss function adapts the margin based on the dissimilarity between pairs:
    - Very similar pairs (S > 0.8): small margin to keep them close
    - Dissimilar pairs (S < 0.2): large margin to push them apart
    - Intermediate similarities: proportional margin based on dissimilarity
    
    Additionally, it applies importance weighting to focus on intermediate similarities
    where discrimination is most important for the embedding space structure.
    
    Parameters
    ----------
    lookup : torch.Tensor
        Lookup table for molecule affinities (normalized to [0,1] by SimilarityCalculator).
    device : torch.device
        Device to run computation on.
    latent_ratio : float
        Ratio of latent dimensions to use (default: 0.75).
    margin : float
        Base margin for moderately similar pairs (default: 2.0).
    weighting_power : float
        Power factor controlling adaptive margin scaling (default: 2.0).
    """

    def __init__(self, lookup: torch.Tensor, device: torch.device, 
                 latent_ratio: float = 0.75, margin: float = 2.0, 
        ...
        # Handle negative similarities by shifting to [0,1] range
        self.sim_offset = 0.0
        if torch.any(self.lookup < 0):
            # If there are negative similarities, compute offset to shift to [0,1]
            min_sim = torch.min(self.lookup)
            max_sim = torch.max(self.lookup)
            print(f"AdaptiveContrastiveAffinityLoss: Detected negative similarities")
            print(f"  Original range: [{min_sim:.4f}, {max_sim:.4f}]")
            # We'll handle this in forward pass to preserve original values
            self.sim_offset = -min_sim.item() if min_sim < 0 else 0.0
            print(f"  Will shift by {self.sim_offset:.4f} to handle negatives")
        
        print(f"AdaptiveContrastiveAffinityLoss initialized:")
        print(f"  Latent ratio: {self.latent_ratio}")
        print(f"  Base margin: {self.margin}")
        print(f"  Weighting power: {self.weighting_power}")
        print(f"  Background-background similarity: {self.background_sim}")
        print(f"  Background-object similarity: {self.background_other_sim}")

    def forward(self, y_true: torch.Tensor, y_pred: torch.Tensor, per_sample: bool = False) -> torch.Tensor:
        """
        Compute adaptive contrastive affinity loss with background handling.
        ...
        -------
        torch.Tensor
            Adaptive contrastive affinity loss.
        """
        try:
            # Ensure inputs are contiguous
            y_true = y_true.contiguous().to(self.device)
            ...
            # Scale distances for numerical stability
            scale_factor = 1.0 / np.sqrt(n_dims_to_use)
            distances = distances * scale_factor
            ...
            if torch.any(both_obj_mask):
                ...
                if torch.any(valid_indices):
                    ...
                    # Get similarities from lookup table
                    valid_similarities = self.lookup[valid_obj_indices1, valid_obj_indices2]
                    
                    # Handle negative similarities by shifting to [0,1] range
                    if self.sim_offset > 0:
                        # Shift and rescale to [0,1]
                        min_val = torch.min(self.lookup)
                        max_val = torch.max(self.lookup)
                        valid_similarities = (valid_similarities - min_val) / (max_val - min_val)
                    
                    # Ensure similarities are in [0,1] range
                    valid_similarities = torch.clamp(valid_similarities, 0.0, 1.0)
                    
                    target_similarities[both_obj_mask][valid_indices] = valid_similarities
            
            # Calculate dissimilarity for adaptive margins
            dissimilarity = 1.0 - target_similarities
            
            # Adaptive margin: larger margin for more dissimilar pairs
            # margin = base_margin * (1 + dissimilarity * weighting_power)
            # This gives:
            # - Similar pairs (S=0.9, D=0.1): margin = base * 1.2
            # - Medium pairs (S=0.5, D=0.5): margin = base * 2.0  
            # - Dissimilar pairs (S=0.1, D=0.9): margin = base * 2.8
            adaptive_margin = self.margin * (1.0 + dissimilarity * self.weighting_power)
            
            # Calculate contrastive loss components
            # Similar pairs: S_{ij} * distance²
            similar_term = target_similarities * (distances ** 2)
            
            # Dissimilar pairs: (1 - S_{ij}) * max(0, adaptive_margin - distance)²
            margin_dist = torch.clamp(adaptive_margin - distances, min=0)
            dissimilar_term = dissimilarity * (margin_dist ** 2)
            
            # Importance weighting: focus on intermediate similarities
            # Peaks at S=0.5, lower weight at extremes (S=0 or S=1)
            # This helps the model learn better discrimination in ambiguous cases
            importance = 1.0 + torch.exp(-10 * (target_similarities - 0.5).abs())
            
            # Combined loss with importance weighting
            losses = (similar_term + dissimilar_term) * importance
            ...
            if self._debug_counter % 100 == 0:
                ...
                print(f"  Distances - Min: {distances.min():.4f}, Max: {distances.max():.4f}, Mean: {distances.mean():.4f}")
                print(f"  Target similarities - Min: {target_similarities.min():.4f}, Max: {target_similarities.max():.4f}, Mean: {target_similarities.mean():.4f}")
                print(f"  Adaptive margins - Min: {adaptive_margin.min():.4f}, Max: {adaptive_margin.max():.4f}, Mean: {adaptive_margin.mean():.4f}")
                print(f"  Importance weights - Min: {importance.min():.4f}, Max: {importance.max():.4f}, Mean: {importance.mean():.4f}")
                print(f"  Similar term - Mean: {similar_term.mean():.4f}")
                print(f"  Dissimilar term - Mean: {dissimilar_term.mean():.4f}")
                print(f"  Total loss - Mean: {losses.mean():.4f}")
